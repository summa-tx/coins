...
mod test {
    ...
    fn it_implements_seq_ops_for_u8() {
        ...
        let exact_len = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::Exactly(buf.len())).unwrap();
        assert_eq!(exact_len.len(), buf.len());
        assert_eq!(input, exact_len);

        // Try to read more than the size of the slice
        let exact_too_long = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::Exactly(buf.len() + 1));
        assert_eq!(exact_too_long.is_err(), true);

        // Read exactly the first element
        let exact_first = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::Exactly(1)).unwrap();
        assert_eq!(exact_first, vec![0]);

        // Read exactly no elements
        let exact_none = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::Exactly(0)).unwrap();
        assert_eq!(exact_none, Vec::<u8>::new());

        // Read at most all of the elements
        let at_most_all = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::AtMost(buf.len())).unwrap();
        assert_eq!(at_most_all, buf.clone());
        //println!("{:?}", at_most)

        // Read at most 10 more elements than exist
        let at_most_more = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::AtMost(buf.len() + 10)).unwrap();
        assert_eq!(at_most_more, buf.clone());

        // Read at most 1 less element than what exists
        let at_most_less = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::AtMost(buf.len() - 1)).unwrap();
        let mut resized = buf.clone();
        resized.resize(buf.len() - 1, 0);
        assert_eq!(at_most_less, resized);

        // Read until the end
        let until_end = u8::read_seq_from(&mut buf.clone().as_slice(), ReadSequenceMode::UntilEnd).unwrap();
