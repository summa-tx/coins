use futures::executor::block_on;
use riemann_core::ser::ByteFormat;
use rmn_bip32::{
    Secp256k1,
    enc::Encoder,
    path::KeyDerivation,
};
use rmn_btc::types::{BitcoinTxIn, Script, ScriptPubkey, WitnessTx, UTXO};
use rmn_ledger_btc::*;

use serial_test::serial;

#[test]
#[serial]
fn it_retrieves_key_info() {
    let backend = Secp256k1::init();
    let app = LedgerBTC::init();
    let result = block_on(app.get_xpub(&(vec![44u32, 44, 44, 44]).into(), Some(&backend))).unwrap();
    println!("{:?}", &result);
    println!(
        "{:?}",
        rmn_bip32::MainnetEncoder::xpub_to_base58(&result.xpub).unwrap()
    );
}

#[test]
#[serial]
fn it_doesnt_sign_without_the_key() {
    let app = LedgerBTC::init();

    let tx = WitnessTx::deserialize_hex("").unwrap();
    let prevout = UTXO {
        outpoint: BitcoinTxIn::deserialize_hex("f1e46af69e3ab97a3b195dbc34af1e2131ec31d53a6e331ab714504d27b6bd940400000000ffffffff").unwrap().outpoint,
        value: 243334728067,
        script_pubkey: ScriptPubkey::deserialize_hex("220020b4d3e699f05e6a2c0d07b06d013508091d291098f9b68dac4a4d24844a2966df").unwrap(),
        spend_script: Some(Script::deserialize_hex("69522102489ec44d0358045c4be092978c40e574790820ebbc3bf069bffc12bda57af27d2102a4bf3a2bdbbcf2e68bbf04566052bbaf45dfe230a7a6de18d97c242fd85e9abc21038d4d2936c6e57f2093c2a43cb17fcf582afb1d312a1e129f900156075a490ae753ae").unwrap()),
    };
    let deriv = KeyDerivation {
        root: [0u8; 4].into(),
        path: vec![44u32 + 2u32.pow(31), 2u32.pow(31), 2u32.pow(31), 0, 1].into(),
    };
    let info = SigningInfo {
        prevout: &prevout,
        deriv: Some(&deriv),
    };
    println!(
        "{:?}",
        block_on(app.get_tx_signatures(&tx, &[info])).unwrap()
    );
}


#[test]
#[serial]
fn it_signs() {
    let app = LedgerBTC::init();
    let backend = Secp256k1::init();
    let xpub = block_on(app.get_xpub(&(vec![]).into(), Some(&backend))).unwrap();

    let tx = WitnessTx::deserialize_hex("").unwrap();
    let prevout = UTXO {
        outpoint: BitcoinTxIn::deserialize_hex("f1e46af69e3ab97a3b195dbc34af1e2131ec31d53a6e331ab714504d27b6bd940400000000ffffffff").unwrap().outpoint,
        value: 243334728067,
        script_pubkey: ScriptPubkey::deserialize_hex("220020b4d3e699f05e6a2c0d07b06d013508091d291098f9b68dac4a4d24844a2966df").unwrap(),
        spend_script: Some(Script::deserialize_hex("69522102489ec44d0358045c4be092978c40e574790820ebbc3bf069bffc12bda57af27d2102a4bf3a2bdbbcf2e68bbf04566052bbaf45dfe230a7a6de18d97c242fd85e9abc21038d4d2936c6e57f2093c2a43cb17fcf582afb1d312a1e129f900156075a490ae753ae").unwrap()),
    };
    let deriv = KeyDerivation {
        root: xpub.derivation.root,
        path: vec![44u32 + 2u32.pow(31), 2u32.pow(31), 2u32.pow(31), 0, 1].into(),
    };
    let info = SigningInfo {
        prevout: &prevout,
        deriv: Some(&deriv),
    };
    println!("");
    println!("");
    println!("WAITING FOR CONFIRMATION");
    println!(
        "{:?}",
        block_on(app.get_tx_signatures(&tx, &[info])).unwrap()
    );
}
