    ...
    E: Bip32Encoder,
{
    ...
    pub fn insert_input(&mut self, index: usize, tx_in: BitcoinTxIn) -> Result<(), PsbtError> {
        ...
        tx.write_to(&mut buf)?;
        self.global_map_mut()
            .insert(GlobalKey::UnsignedTx.into(), buf.into());
    ...
    E: Bip32Encoder,
{
    ...
    fn from_tx<Tx: BitcoinTransaction>(tx: &Tx) -> Psbt<T, E> {
        ...
        for (i, witness) in tx.witnesses().iter().enumerate() {
            inputs[i].insert_witness(witness.to_vec());
        }
...
pub type MainnetPbst = Psbt<MainnetEncoder, bip32::enc::MainnetEncoder>;

/// A PSBT Parameterized for testnet
pub type TestnetPbst = Psbt<TestnetEncoder, bip32::enc::TestnetEncoder>;
...
mod test {
    ...
    macro_rules! assert_err {
        ($hex:literal, $err:pat) => {
            match MainnetPbst::deserialize_hex($hex) {
                Err($err) => {}
    ...
    fn it_deserializes_without_modifying() {
        ...
        for case in &valid_cases {
            let p = MainnetPbst::deserialize_hex(case).unwrap();
    ...
    fn invalid_psbt_network_tx() {
        ...
        match MainnetPbst::deserialize_hex(psbt) {
    ...
    fn it_instantiates_from_txns() {
        ...
        let psbt = MainnetPbst::from_tx(&tx);
    ...
    fn it_decodes_a_thing() {
        ...
        assert_eq!(
            b64,
            MainnetPbst::deserialize_base64(&b64)
                .unwrap()
                .serialize_base64()
        );
    }
